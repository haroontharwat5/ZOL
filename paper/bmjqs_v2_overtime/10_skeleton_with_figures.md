...
- Overtime is the binding economic cost in OR operations, weighted 1.5-2x the cost of idle time [1,2].
- **Staff harm.** Overtime and long shifts are associated with poorer perceived care quality and higher patient-safety risk in a 12-country European nurse workforce study [3]. The companion study found that 12-hour shifts were associated with burnout and intent to leave [4]. The most recent direct evidence links mandatory overtime to intent to leave in a 2024 cross-sectional study of 397 nurses [5]. The distinction between planned and unplanned overtime matters: unplanned overtime is more harmful than planned long shifts [23,24].
- **Patient harm.** After-hours surgery carries elevated mortality in a 2020 meta-analysis (adjusted OR 1.16, 95% CI 1.06-1.28) [6], recently replicated in a 281,717-patient propensity-matched South Korean cohort [7]. Each intraoperative anaesthesia handover raises the odds of a major composite complication [8]. A 2025 UK national safety investigation named staff fatigue as a direct patient-safety problem [9]. Overtime hours have been directly associated with patient-safety indicators across 70 US hospitals [10].
...
**References for §1:** [1] Dexter 2004, [2] McIntosh 2006, [3] Griffiths 2014, [4] Dall'Ora 2015, [5] Cho/Bae 2024, [6] Cortegiani 2020, [7] Oh 2025, [8] Saager 2014, [9] HSSIB 2025, [10] Pittman 2025, [23] Dall'Ora 2016, [24] Dall'Ora 2020.
...
- Per Bauer et al. (2020) glossary [11]: planned duration, observed duration, planning deviation, scheduled start, actual start, start-time deviation, room-out time, overtime flag (room-out > 16:30), overtime minutes, planned room vs actual room (room-swap flag), urgency at planning (elective vs niet-electief), shift label (day 08:00-16:30; evening 16:30-22:00; night 22:00-08:00).
- Metric choice justified by the inconsistency documented across 47 OR metrics in a recent systematic review [12] and the argument that aggregate utilisation hides room-level reality [13].

### 2.4 Overtime definition (80 words)

- Overtime = case still in the room past 16:30 on the calendar day, or any case ending in the evening or night shift. Shift-based, not duration-based.
- Chosen because the staffing change at 16:30 is the operational event that gives overtime its meaning at this site. Aligns with Bauer 2020 §3.4 [11] and the hospital's own "area under the curve" framing.
...
**Headline numbers:**
- 96,044 cases, 8,024 ran past 16:30 (8.4%). Mean overtime 59 min; median 38 min; P95 193.8 min.
...
- GO10: 32.9% overtime, mean 154 min, P95 327 min. One in every three cases runs into after-hours. Likely cardiac complex surgery (to be confirmed with hospital).
...
- Elective: 84,028 cases, 7% after-hours, mean OT 4.1 min, P95 18 min.
- Non-elective: 12,016 cases, 18% after-hours, mean OT 10.5 min, P95 67.2 min.
- Urgent cases run after-hours at more than twice the rate and with longer overruns. But elective cases are seven times more numerous, so the absolute pool of after-hours minutes is dominated by the elective programme.
...
- Idle time: mean 9.2 min, median 7 min, P95 25 min. Turnover is fast and consistent. Idle time between cases is not the bottleneck. One sentence. [Supplementary Figure S1.] [32]
...
- This kills the first-case-on-time-start (FCOTS) narrative: the worst-late-start room has zero overtime, and the worst-overtime room is mid-pack on punctuality. Consistent with Pandit et al. (2012) finding that R-squared between start and finish times is 0.04-0.08 [17]. [Supplementary Table S2.]
...
- **FCOTS contrast.** The conventional view holds that first-case-on-time starts drive end-of-day performance [16]. Our data contradict this: GEG1 has the worst first-case punctuality (90% late) and zero overtime; GO10 is mid-pack on punctuality and worst on overtime. Pandit et al. (2012) showed the same disconnect (R-squared 0.04-0.08 between start and finish times) [17]. FCOTS is a useful discipline metric but not a reliable lever for overtime reduction.
...
- **Literature parallel.** Wachtel & Dexter (2009) described cascading in a theoretical OR model: first-case tardiness propagating through the day [19]. Fugener et al. (2017) added a behavioural dimension: surgeons compress their work when they sense they are approaching a boundary [20]. Our -22 min duration deviation is consistent with that compression effect.
- **Reframing the intervention.** If the mechanism is cascading rather than long-running individual cases, the intervention point is earlier in the day: scheduling density, buffer placement, and urgent-case routing — not end-of-list management. Joseph et al. (2019) documented the same escalation pattern in a different OR context [21]. Abdelfattah et al. (2020) linked flow disruptions directly to outcomes [22].

**References for §4.2:** [19] Wachtel & Dexter 2009, [20] Fugener 2017, [21] Joseph 2019, [22] Abdelfattah 2020.

### 4.3 Finding 3 — Urgent-elective interaction (150 words)

- Urgent cases run after-hours at more than twice the rate of elective cases (18% vs 7%). But elective cases are seven times more numerous, so the absolute pool of after-hours minutes is dominated by the elective programme. Overlap occurs on 70% of days, adding 30 min to elective start times.
...
- The unpredictability of the overlap is the staff-harm dimension: unplanned overtime is more harmful than planned long shifts [23].
...
- **Staff harm.** Overtime and long shifts are associated with burnout, reduced perceived care quality, and intent to leave [3,4,5]. Unpredictable overtime is worse than planned long hours [23,24]. In a Flemish nursing workforce, workload and schedule control dominate burnout predictors [25]. The HSSIB 2025 investigation named staff fatigue as a patient-safety problem [9].
- **Patient harm.** After-hours surgery carries elevated mortality [6,7], though the effect size is debated [26]. Each intraoperative handover raises complication risk [8], though recent sub-specialty data are mixed [27]. Structured handover programmes reduce adverse events [28]. Shifts exceeding 24 hours produce 36% more serious errors [29]. Overtime hours have been directly associated with patient-safety indicators across 70 US hospitals [10].
...
**References for §4.4:** [3] Griffiths 2014, [4] Dall'Ora 2015, [5] Cho/Bae 2024, [6] Cortegiani 2020, [7] Oh 2025, [8] Saager 2014, [9] HSSIB 2025, [10] Pittman 2025, [23] Dall'Ora 2016, [24] Dall'Ora 2020, [25] Van Bogaert 2017, [26] Sakurai 2025, [27] Guerra-Londono 2025, [28] Starmer/I-PASS 2014, [29] Barger 2006.
...
5. **Confounding by case complexity.** GO10 likely handles the most complex cardiac surgery. Its high overtime may partly reflect irreducible procedural duration. We describe this but cannot adjust without procedure-level risk scores.
...
5. Cho (Bae) J, Cho HJ, et al. Nurse staffing, work hours, mandatory overtime, and turnover in acute care hospitals affect nurse job satisfaction, intent to leave, and burnout. *Int J Public Health* 2024;69:1607068.
...
13. Zhang J, Dunstan M, Pandit JJ. A tutorial on "capped utilisation" as a metric in NHS England's Model Hospital operating theatres database. *Anesthesiol Perioper Sci* 2024. DOI: 10.1007/s44254-024-00073-3.
...
22. Abdelfattah E, et al. Workflow disruptions and outcomes. *BMJ Qual Saf* 2020;29:1009-17.
