...
numbered_qs = [
    ...
    '2. **Which operational factors — duration overruns, shift displacement, urgent-elective interaction, and first-case punctuality — are associated with overtime?** This tests which mechanisms identified in the literature are visible in our data, to inform where scheduling interventions should be directed.',
]
...
add_paragraph('Each case was assigned to a shift based on its actual room-in time: day (08:00 to 16:30), evening (16:30 to 22:00), or night (22:00 to 08:00). A case was flagged as overtime if its room-out fell after the end of its assigned shift; overtime minutes equal the positive difference between room-out and shift end. We chose this shift-based definition because the staffing change at each boundary is the operational event that makes overtime consequential at this site, and because it aligns with Bauer et al.[18] Room-level overtime, rather than aggregate utilisation, was used as the primary metric, since aggregate measures can mask room-level operational problems.[16,19]', space_after=6)
add_paragraph('All analyses are descriptive; we did not fit causal or inferential models. For RQ1, we computed overtime rate, mean, median, and 95th percentile, stratified by room, weekday, year, and shift. For RQ2, we examined duration deviation by planned-duration bucket using coefficients of variation,[20,21] shift displacement (cases performed in a different shift than originally planned), urgency mix and timing, urgent-elective interaction (defined and reported in Results), first-case start-time deviation per room, inter-case idle time, and room swaps. This study is reported following the STROBE guidelines for observational studies.', space_after=6)
...
add_paragraph('In this single-centre analysis of 79,352 cases, overtime was not a diffuse hospital-wide phenomenon. The aggregate rate (9.7%) hid a near ten-fold spread across rooms (3.5 to 32.9%). The factors most visibly associated with the room-level pattern were case-mix complexity and routine urgent-elective overlap; first-case punctuality and inter-case idle time were not. We discuss each in turn against the existing literature, draw out the implications for quality improvement, and end with the study\'s limitations.', space_after=6)
...
add_paragraph('The procedures with the largest planning deviations — complex cardiac and oncology cases — clustered in the rooms with the highest overtime. This concentration suggests that case-mix complexity, rather than scheduling inefficiency alone, accounts for much of the room-level variation. Wachtel and Dexter described, in a large operational dataset, how tardiness grows as cumulative duration uncertainty accumulates through the day,[26] and Fugener et al. documented systematic biases in surgeons\' duration estimates that compound across a list.[27] The 5.2% of cases displaced into a later shift, with a mean start delay of just over six hours, fits this cumulative-delay pattern: the cases themselves run on time or shorter, yet they land in the period when staffing has stepped down.', space_after=6)
...
add_paragraph('Clinical implication. The overtime we document is the same exposure that other studies have linked to staff fatigue and to elevated after-hours mortality and complication risk (Introduction; references 2-13). Whether those associations hold in this cohort is the question Phase 2 of this programme will address by linking the operational patterns we describe to outcome data.', space_after=6)
