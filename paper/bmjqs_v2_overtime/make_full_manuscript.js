...
const children = [
  ...
  para([
    ...
    t(' Handovers carry risk of their own: a case that crosses the shift boundary continues under staff who were not present at its start, and the incidence of major complications rises with each additional intraoperative anaesthesia transition, from 8.8% with none to 21.2% with four or more.'), s('5'),
  ]),
  para([
  ...
  para([
    t('An OR department that wants to reduce overtime first has to know where it occurs, and the figures routinely available to management seldom show this. Established efficiency scoring operates at the level of the whole OR suite,'), s('8'),
    t(' published performance metrics lack standardised definitions,'), s('9'),
    t(' and aggregation can mislead: NHS England’s capped-utilisation metric ignores operating time in late finishes, which is precisely the time overtime consists of.'), s('10'),
    t(' Prior analyses of overtime itself treat it as a department-level number or as a cost parameter in staffing models.'), s('11'),
    t(' Where overtime sits within a department, room by room, and which operational factors are associated with it, the literature does not answer. The records needed to answer these questions already exist: OR information systems log room entry and exit times, planned durations, and urgency classifications in the course of daily operations.'), s('12'),
  ]),
  ...
  para([
    t('Time beyond plan goes by different names in the literature, depending on the reference point. Measured against the room’s allocated block time, it is overutilisation.'), s('12'),
    ...
    t(' This study uses the roster as the reference point, applied at room level, because the change of nursing staff at each shift end is the operational event that makes a late-running case consequential at this site.'),
  ]),
  para([t('Each case was assigned to a shift by its actual room-in time, following the hospital’s shift boundaries: day (08:00 to 16:30), evening (16:30 to 22:00), and night (22:00 to 08:00). One reclassification was applied: cases entering the room between 07:30 and 08:00 and remaining past 08:00 (n = 6,153) were treated as day-shift cases, since these are early starts of the day programme rather than night activity. The same rules were applied on weekends. A case was flagged as overtime if its room-out fell after the end of its assigned shift, and its overtime minutes equal the positive difference between room-out and shift end. A case was classified as a scheduled crossing when its planned end time already fell after the end of its planned shift; overtime cases that were not scheduled crossings are referred to as unplanned overruns.')]),
  para([
    t('We measured overtime at room level because aggregate utilisation measures can misrepresent the performance of individual rooms.'), s('10'),
    t(' Because published OR performance metrics lack standardised definitions,'), s('9'),
    t(' each metric used in this study is defined explicitly.'),
  ]),
  para([
    t('In line with the study’s aim of showing what routinely recorded data can contribute to overtime monitoring, the analyses are descriptive apart from two rank correlations: we report rates, distributions, and shares as observed, we fitted no regression or causal models, and we draw no causal conclusions from the associations reported. For RQ1, we computed the overtime rate, mean, median, and 95th percentile, stratified by room, weekday, year, and shift, together with the split between scheduled crossings and unplanned overruns. For RQ2, we examined duration deviation by planned-duration bucket using the coefficient of variation, defined as the standard deviation of duration divided by its mean, which places cases of different lengths on a comparable scale.'), s('1,13'),
  ...
  para([
    ...
    t(' Delays have also been shown to build up over an operating list as the scheduled time before each case grows.'), s('16'),
