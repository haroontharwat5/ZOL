...
# ---- Remove observations where DurationMinutes == 0 ----
if ("DurationMinutes" %in% names(data)) {
  data <- data %>% filter(DurationMinutes != 0)
}

# ---- Remove observations where PlannedDurationMinutes > 24 hours ----
if ("PlannedDurationMinutes" %in% names(data)) {
  data <- data %>% filter(PlannedDurationMinutes < 1440)
}

# ---- change planned duration of 0 minutes to NA ----
data <- data %>%
  mutate(
    PlannedDurationMinutes = ifelse(PlannedDurationMinutes == 0, NA, PlannedDurationMinutes)
  )

# ---- Remove unrealistic planned durations ----
data <- data %>%
  filter(!(UrgencyType == "electief" & PlannedDurationMinutes <= 1))

# ---- Remove unrealistic actual durations ----
data <- data %>%
  filter(!(UrgencyType == "electief" & DurationMinutes <= 1))

# ---- Remove negative planned durations ----
data <- data %>%
...
# ---- Remove cases with unrealistic start_diff (> 2 days difference) ----
data <- data %>%
  filter(is.na(start_diff) | abs(start_diff) <= 3000)
...
# ---- Remove cases with unrealistic end_diff (> 2 days difference) ----
data <- data %>%
  filter(is.na(end_diff) | abs(end_diff) <= 3000)
...
# ---- Remove unrealistic long deviations ----
data <- data %>%
  filter(
    !(
...
# ---- Remove procedures that are not under anesthesia ----
data <- data %>%
  filter(!ProcedureName %in% c(
    "OZ Colonscopie/Anesthesie",
    "HYSTEROSCOPIE OPERATIEF BIPOLAIR",
    "PICK-UP",
    "Ingreep sedatie",
    "KABINETCHIRURGIE PLASTISCHE",
    "E.R.C.P.",
    "BLEPHAROPLASTIE BOVEN"
  ))

# ---- Keep only DAY and HOS admissions ----
data <- data %>%
  filter(AdmissionType %in% c("DAG", "HOS"))

# ---- Remove unrealistic short deviations ----
data <- data %>%
  filter(
    !(
      UrgencyType == "electief" &
...
data <- data %>%
  ...
  mutate(
    ...
    gap_time  = case_when(
      ...
      gap_raw > 60                  ~ NA_real_,    # te lange idle -> niet als efficiëntie
      TRUE                           ~ gap_raw
    )
  ) %>%
  ungroup() %>%
...
# ---- Keep only GO operating rooms (GO followed by numbers) for Genk ----
genk <- genk %>%
  filter(str_detect(PlannedOR, "^GO[0-9]+$"))

# ---- Keep only GO operating rooms for Genk (based on Actual OR) ----
genk <- genk %>%
  filter(str_detect(ActualOR, "^GO[0-9]+$"))
